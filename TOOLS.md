...
**本项目所有 Python 脚本必须通过虚拟环境执行。**

- 路径：`venv/bin/python3 <脚本路径>`
- 禁止直接使用系统 `python3` 运行任何项目脚本
- 新增 Python 脚本后，同样必须通过 `venv/bin/python3` 调用
...
1.  **召回匹配模板句型**：调用 `venv/bin/python3 recall.py <word>` 召回匹配句型
